...
path_to_YASRA_fSAM = './SRR8528336_reads.fq_ref-at.fasta_Wed-May--6-142648-2020/YASRA_related_files/alignments_SRR8528336_reads.fq_ref-at.fasta.txt'
path_to_exons = './results/exons/txt'

# Create target Directory if don't exist
if not os.path.exists(path_to_exons):
    os.mkdir(path_to_exons)
    print("Directory ", path_to_exons, " Created ")
else:
    print("Directory ", path_to_exons, " already exists")
    ...
    for myline in myfile:
        if myline.startswith('>'):
            ...
            if contig != contig_name_temporary:
                f = open(path_to_exons + "/" + contig_number + "_" + reference_genome + ".txt", "w+")
                ...
                f = open(path_to_exons + "/" + contig_number + "_" + reference_genome + ".txt", "a+")
                f.write(myline)
                f.close()
#
    f = open(path_to_exons + "/number_of_contigs_and_exons.txt", "w+")
